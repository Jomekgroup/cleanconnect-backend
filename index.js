...
app.post('/api/reviews', protect, async (req, res) => {
  ...
  try {
    ...
    const cleanerName = cleanerRes.rows[0]?.full_name || 'Cleaner';
    const clientRes = await pool.query('SELECT full_name FROM users WHERE id = $1', [req.user.id]);
    const reviewerName = clientRes.rows[0]?.full_name || 'Client';

    const result = await pool.query(
      `INSERT INTO reviews (
        cleaner_id, cleaner_name, reviewer_id, reviewer_name, rating, comment, booking_id, created_at
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW()) RETURNING *`,
      [cleanerId, cleanerName, req.user.id, reviewerName, rating, comment, bookingId]
    );
...
app.get('/api/chats/:chatId/messages', protect, async (req, res) => {
  try {
    ...
    res.json(result.rows);
  } catch (error) { handleError(res, error, 'Fetching messages failed'); }
});
...
app.post('/api/chats/:chatId/messages', protect, async (req, res) => {
  ...
  try {
    const chatRes = await pool.query(
      'SELECT * FROM chats WHERE id = $1 AND (client_id = $2 OR cleaner_id = $2)',
      [req.params.chatId, req.user.id]
    );
    if (chatRes.rows.length === 0) return res.status(404).json({ message: 'Chat not found or unauthorized' });

    const messageRes = await pool.query(
      'INSERT INTO messages (chat_id, sender_id, content, created_at) VALUES ($1, $2, $3, NOW()) RETURNING *',
      [req.params.chatId, req.user.id, content]
    );

    await pool.query(
      'UPDATE chats SET updated_at = NOW(), last_message_content = $1 WHERE id = $2',
      [content, req.params.chatId]
    );

    res.status(201).json(messageRes.rows[0]);
  } catch (error) { handleError(res, error, 'Sending message failed'); }
});
...
app.post('/api/subscriptions/proof', protect, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE users SET
      pending_subscription = COALESCE($1, pending_subscription),
      subscription_receipt = COALESCE($2, subscription_receipt)
      WHERE id = $3 RETURNING *`,
      [pendingSubscription, JSON.stringify(subscriptionReceipt), req.user.id]
    );
    await sendEmail('admin@myapp.com', 'New Subscription Proof Submitted', `User ${req.user.id} submitted proof for ${pendingSubscription}.`);
    res.json(result.rows[0]);
...
app.get('/api/admin/dashboard', protect, admin, async (req, res) => {
  try {
    ...
    const completedBookings = (await pool.query("SELECT COUNT(*) FROM bookings WHERE status = 'Completed'")).rows[0].count;
    const totalRevenue = (await pool.query("SELECT COALESCE(SUM(total_amount), 0) FROM bookings WHERE payment_status = 'Confirmed' OR payment_status = 'Pending Payout'")).rows[0].coalesce;

    res.json({
      clientCount: parseInt(clientCount), cleanerCount: parseInt(cleanerCount),
      pendingBookings: parseInt(pendingBookings), completedBookings: parseInt(completedBookings),
      totalRevenue: parseFloat(totalRevenue)
    });
  } catch (error) { handleError(res, error, 'Admin dashboard failed'); }
});

app.post('/api/admin/users/:id/suspend', protect, admin, async (req, res) => {
  const { isSuspended } = req.body;
  try {
    const result = await pool.query(
      'UPDATE users SET is_suspended = $1 WHERE id = $2 RETURNING *',
      [isSuspended, req.params.id]
    );
    res.json(result.rows[0]);
  } catch (error) { handleError(res, error, 'User suspension failed'); }
});

app.post('/api/admin/subscriptions/:id/approve', protect, admin, async (req, res) => {
  try {
    const userRes = await pool.query('SELECT pending_subscription FROM users WHERE id = $1', [req.params.id]);
    const tier = userRes.rows[0]?.pending_subscription;

    if (!tier) return res.status(400).json({ message: 'No pending subscription to approve' });

    const result = await pool.query(
      "UPDATE users SET subscription_tier = $1, pending_subscription = NULL, subscription_receipt = NULL WHERE id = $2 RETURNING *",
      [tier, req.params.id]
    );
    await sendEmail(result.rows[0].email, 'Subscription Approved', `Your ${tier} subscription has been approved.`);
    res.json(result.rows[0]);
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO support_tickets (
        user_id, user_name, user_email, subject, description, priority, status, created_at
      ) VALUES ($1, $2, $3, $4, $5, $6, 'Open', NOW()) RETURNING *`,
      [req.user.id, full_name, email, subject, description, priority]
    );

    await sendEmail('admin@myapp.com', `New Support Ticket: ${subject}`, `Priority: ${priority}\nDescription: ${description}`);
    res.status(201).json(result.rows[0]);
